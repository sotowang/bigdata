...
- 偏向锁
  ...
  - 升级过程
    ...
    - 偏向锁不会主动释放锁，线程1再次获取锁时，需比较当前线程threadID和对象头中threadID是否一致
      - 若一致，则无需用CAS加锁，解锁；
...
* 对代码块同步
  * 当monitor被占用时处于锁定状态，线程执行monitorenter指令尝试获取monitor所有权
    * 若monitor进入数为0，线程进入monitor，然后将进入数设为1
    * 若线程已占有monitor，只是重新进入，则monitor进入数加1
